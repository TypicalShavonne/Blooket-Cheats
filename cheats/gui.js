...
(() => {
  const cheat = async () => {
    ...
    let userData =
      (await Object.values(
        ...
        .exports.a.me({})) || {};
    let settings,
      settingsKey = btoa(userData.name || "real"),
      guiId = btoa(userData.id || "lmfao").replaceAll(/(=|\/|\.)/g, "");
    const Settings = {
      data: null,
      setItem(k, v) {
        k.split(".").reduce(
    ...
    const guiWrapper = createElement(
      ...
      (gui = createElement(
        ...
        (controls = createElement("div", {
          ...
          innerText: ((
            { ctrl: ctrlHide, shift: shiftHide, alt: altHide, key: keyHide } = {
              ctrl: true,
              key: "e",
            },
            {
            ...
            Settings.data.hide || { ctrl: true, key: "e" },
            Settings.data.close || { ctrl: true, key: "x" }
          ),
          update: (
            { ctrl: ctrlHide, shift: shiftHide, alt: altHide, key: keyHide } = {
              ctrl: true,
        ...
        createElement("div", {
          ...
          innerHTML: "GitHub - TypicalShavonne",
        }),
          ...
          createElement("button", {
            ...
            onclick: (function () {
              ...
              return () => {
                ...
                guiWrapper.style.top = `${
                  parseInt(guiWrapper.style.top) +
                  (guiWrapper.offsetHeight - 55) * (hidden ? -1 : 1)
                }px`;
                guiWrapper.style.left = `${
                  parseInt(guiWrapper.style.left) +
                  (guiWrapper.offsetWidth - 165) * (hidden ? -1 : 1)
                }px`;
        ...
        (cheatContainer = createElement(
          "div",
          {
            ...
            innerHTML:
              '<span style="text-shadow: 1px 1px rgb(0 0 0 / 40%)">Cheats</span>',
          },
          createElement("a", {
            className: "bigButton",
            style: {
    ...
    for (const oldGui of document.querySelectorAll("#" + guiId))
      oldGui.remove();
    ...
    function addMode(mode, img, cheats, nameOnly) {
      const button = createElement("div", {
        ...
        innerHTML:
          (typeof img == "string"
            ? `<img style="height: 30px; margin-right: 5px" src="${img}">`
            : img
            ? img
            : "") + mode,
    ...
    async function setCheats(mode, scripts, nameOnly) {
      ...
      for (let i = 0; i < scripts.length; i++) {
        let { name, description, type, inputs, enabled, run, element } =
          scripts[i];
        if (!element) {
          ...
          button.onclick = function ({ target, key }) {
            if (
              target != button &&
              !target.classList.contains("cheatName") &&
              !(key == "Enter" && target.classList.contains("cheatInput"))
            )
              return;
            ...
            for (let i = 0; i < inputs.length; i++) {
              ...
              if (type == "options" && options?.length) {
                ...
                input.onclick = async () => {
                  ...
                  locked = true;
                  input.data = await inputs[i].function(
                    (e) => (input.value = e + "...")
                  );
                  locked = false;
                ...
                return new Promise((r) => {
                  r(
                    Object.keys(
                      ...
                      ).find((x) => !isNaN(x?.exports?.a?.Space)).exports.a ||
                        {}
                    )
                  );
                });
          ...
          run: async function (id, name, amount, b, bg) {
            ...
            for (let i = 1; i <= amount; i++) {
              (async () => {
                ...
                } = await axios.put("https://fb.blooket.com/c/firebase/join", {
                  id,
                  name: ign,
                });
                if (!success) return;
        ...
        },
        {
          ...
          run: function () {
            ...
            axios
              ...
              .catch(
                (e) => (alert("There was an error user data!"), console.info(e))
              );
          },
        },
        {
                        ...
                        for (var u = 0; u < 3; u++) {
                          ...
                          d &&
                            d.spawn.apply(
                              d,
                              Object.values(
                                Z(u % 2 == 0 ? "left-diamond" : "right-diamond")
              ...
              this.data = setInterval(() => {
                ...
                if (state.stage == "hack")
                  for (const button of document.querySelector(
                    ...
                    button.innerText == state.correctPassword && button.click();
              }, 50);
            } else {
              this.enabled = false;
              clearInterval(this.data);
                  ...
                  if (stateNode.state.stage === "prize") {
                    ...
                    for (let i = 0; i < stateNode.state.choices.length; i++) {
                      ...
                      switch (type) {
                        case "gold":
                          value =
                            stateNode.state.gold + val || stateNode.state.gold;
                          break;
                        case "multiply":
                        case "divide":
                          value =
      ...
      alerts: [
        {
          ...
          addLog(message, color) {
            return this.element.firstChild.prepend(
              createElement(
                "li",
                { style: { margin: "5px" } },
                createElement("span", {
                  style: { color: color || "var(--textColor)" },
            ...
            for (const { blook, name, value } of standings) {
              this.leaderboard.append(
                createElement(
                  ...
                  createElement("img", {
                    src:
                      this.blookData?.[blook]?.url || this.blookData.Black.url,
                    alt: blook,
                    draggable: false,
                    style: {
                      height: "45px",
          ...
          parseNumber(num = 0) {
            ...
            if (coeffIndex < coeffs.length) {
              ...
              for (let i = 3; i >= 1; i--) {
                rounded = parseFloat(
                  (0 !== coeffIndex
                    ? num / Math.pow(1e3, coeffIndex)
                    : num
                  ).toPrecision(i)
                );
                if (
                  rounded.toString().replace(/[^a-zA-Z 0-9]+/g, "").length <= 3
                )
                  break;
              }
          ...
          addLeaderboard() {
            ...
            ).find((x) => x.exports?.a?.Alice && x.exports?.a?.Alien).exports.a;
            this.element.append(
              (this.leaderboardEl = createElement(
                "div",
                {
          ...
          async connect() {
            try {
              ...
              const blooks = (this.blookData = Object.values(
                webpackJsonp.push([
                  [],
                  {
                    [""]: (_, a, b) => {
                ...
                (x) => x.exports?.a?.Alice && x.exports?.a?.Alien
              ).exports.a);
              const gamemode = this.getGamemode();
              const factoryGlitches = {
                lb: "Lunch Break",
                as: "Ad Spam",
                e37: "Error 37",
                nt: "Night Time",
    ...
    addMode(
      '<span style="font-size: 17px">Deceptive Dinos</span>',
      [
        `<img style="height: 30px; margin-left: 8px; margin-right: 12px" src="https://media.blooket.com/image/upload/v1655161325/Media/survivor/Dog.svg">`,
      ],
      Cheats.dinos
    );
    addMode(
      '<span style="font-size: 18px">Tower Defense</span>',
      [
        `<img style="width: 30px; margin-right: 5px" src="https://media.blooket.com/image/upload/v1657235025/Media/survivor/Laser_Lvl1.svg">`,
      ],
      Cheats.defense
    );
    addMode(
      '<span style="font-size: 16px">Tower Defense 2</span>',
      [
        `<img style="width: 30px; margin-right: 5px; rotate: 45deg" src="https://media.blooket.com/image/upload/v1593095354/Media/defense/missile.svg">`,
      ],
      Cheats.defense2
    );
    addMode(
    ...
    addMode(
      '<span style="font-size: 19px">Fishing Frenzy</span>',
    ...
    addMode(
      '<span style="font-size: 17px">Tower of Doom</span>',
      [
        `<img style="height: 30px; margin-left: 5px; margin-right: 10px" src="https://media.blooket.com/image/upload/v1657235023/Media/survivor/cards-05.svg">`,
      ],
      Cheats.doom
    );
    addMode(
      '<span style="font-size: 18px">Crazy Kingdom</span>',
    ...
    addMode(
      '<span style="font-size: 18px">Monster Brawl</span>',
      [
        `<img style="height: 28px; margin-left: 5px; margin-right: 8px" src="https://media.blooket.com/image/upload/v1655233787/Media/survivor/xp/Blue_xp_2.svg">`,
      ],
      Cheats.brawl
    );
    addMode(
    ...
    function close() {
      ...
      for (const category in Cheats)
        for (const cheat of Cheats[category]) if (cheat.enabled) cheat.run();
      Object.keys(Cheats).forEach((mode) =>
        Cheats[mode].forEach(
          (cheat) => cheat.enabled && (cheat.run(), setCheats(...currentMode))
        )
      );
    ...
    guiWrapper.addEventListener("mousemove", (e) => {
      if (
        e.target.className != "cheatName" &&
        e.target.className != "scriptButton"
      ) {
        if (tooltip.style.opacity != "0") {
          tooltip.animate([{ opacity: 0.9 }, { opacity: 0 }], {
            duration: 200,
          });
    ...
    function keydown(e) {
      ...
      if (
        ...
        e.preventDefault();
        guiWrapper.style.display =
          guiWrapper.style.display === "block" ? "none" : "block";
      } else if (
  ...
  let img = new Image();
  img.src =
    "https://raw.githubusercontent.com/TypicalShavonne/Blooket-Cheats/main/autoupdate/timestamps/gui.png?" +
    Date.now();
  ...
  img.onload = function () {
    ...
    if (parseInt(time) <= 1696542129455 || iframe.contentWindow.confirm(error))
      cheat();
  };
