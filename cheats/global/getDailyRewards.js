...
(() => {
  const cheat = async () => {
    ...
    if (!location.href.includes("play.blooket.com"))
      ...
      await axios.post("https://play.blooket.com/api/playersessions/landings", {
        t,
      });
      await axios.get("https://play.blooket.com/api/playersessions/questions", {
        params: { t },
      });
      const {
        name,
  ...
  let img = new Image();
  img.src =
    "https://raw.githubusercontent.com/TypicalShavonne/Blooket-Cheats/main/autoupdate/timestamps/global/getDailyRewards.png?" +
    Date.now();
  ...
  img.onload = function () {
    ...
    if (parseInt(time) <= 1696542129449 || iframe.contentWindow.confirm(error))
      cheat();
  };
