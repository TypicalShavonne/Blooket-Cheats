...
(() => {
  const cheat = async () => {
    setInterval(() => {
      ...
      let elements = Object.fromEntries(
        [...document.querySelectorAll("[class^=styles__statContainer]")].map(
          (container, i) => [stats[i], container]
        )
      );
  ...
  let img = new Image();
  img.src =
    "https://raw.githubusercontent.com/TypicalShavonne/Blooket-Cheats/main/autoupdate/timestamps/kingdom/choiceESPLoop.png?" +
    Date.now();
  ...
  img.onload = function () {
    ...
    if (parseInt(time) <= 1693429947509 || iframe.contentWindow.confirm(error))
      cheat();
  };
