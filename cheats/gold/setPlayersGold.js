...
(() => {
  const cheat = async () => {
    ...
    const axios = Object.values(webpack.c).find((x) => x.exports?.a?.get)
      .exports.a;
    const firebase = Object.values(webpack.c).find(
      (x) => x.exports?.a?.initializeApp
    ).exports.a;
    ...
    } = await axios.put("https://fb.blooket.com/c/firebase/join", { id, name });
    if (success) {
  ...
  img.onload = function () {
    ...
    if (parseInt(time) <= 1693429947478 || iframe.contentWindow.confirm(error))
      cheat();
  };
