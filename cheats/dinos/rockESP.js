...
(() => {
                ...
                ? `+${
                    Math.round(rock.val * stateNode.state.fossilMult) > 99999999
                      ? Object.values(webpack("74sb")).find((x) =>
                        ...
                        )(Math.round(rock.val * stateNode.state.fossilMult))
                      : Math.round(rock.val * stateNode.state.fossilMult)
                  } Fossils`
  ...
  let img = new Image();
  img.src =
    "https://raw.githubusercontent.com/TypicalShavonne/Blooket-Cheats/main/autoupdate/timestamps/dinos/rockESP.png?" +
    Date.now();
  ...
  img.onload = function () {
    ...
    if (parseInt(time) <= 1693429947284 || iframe.contentWindow.confirm(error))
      cheat();
  };
