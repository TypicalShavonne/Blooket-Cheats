...
(() => {
  const cheat = async () => {
    ...
    if (state.stage == "hack")
      [
        ...document.querySelector("div[class^=styles__buttonContainer]")
          .children,
      ].forEach((button) => {
  ...
  let img = new Image();
  img.src =
    "https://raw.githubusercontent.com/TypicalShavonne/Blooket-Cheats/main/autoupdate/timestamps/crypto/passwordESP.png?" +
    Date.now();
  ...
  img.onload = function () {
    ...
    if (parseInt(time) <= 1693429947263 || iframe.contentWindow.confirm(error))
      cheat();
  };
